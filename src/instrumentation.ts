export async function register() {
  if (process.env.NEXT_RUNTIME === "nodejs") {
    ...
    const isPlaceholder = !headers || headers.includes("your-api-key-here");
    ...
    const { ATTR_SERVICE_NAME, SEMRESATTRS_DEPLOYMENT_ENVIRONMENT } =
      await import("@opentelemetry/semantic-conventions");
    const { getAppEnv } = await import("@/lib/env");

    const resource = resourceFromAttributes({
      [ATTR_SERVICE_NAME]:
        process.env.OTEL_SERVICE_NAME ?? "effect-talk-website",
