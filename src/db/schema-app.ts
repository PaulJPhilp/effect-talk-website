...
export const analyticsEvents = pgTable(
  ...
  (table) => [index("idx_events_type").on(table.eventType)]
);
...
export const tourLessons = pgTable(
  "tour_lessons",
  {
    id: uuid("id").primaryKey().defaultRandom(),
  ...
  (table) => [uniqueIndex("idx_tour_lessons_order").on(table.orderIndex)]
);
...
export const tourSteps = pgTable(
  "tour_steps",
  {
    ...
    lessonId: uuid("lesson_id")
      .notNull()
      .references(() => tourLessons.id, { onDelete: "cascade" }),
    orderIndex: integer("order_index").notNull(),
    title: text("title").notNull(),
    instruction: text("instruction").notNull(),
    conceptCode: text("concept_code"),
    conceptCodeV4: text("concept_code_v4"),
    conceptCodeLanguage: text("concept_code_language").default("typescript"),
    solutionCode: text("solution_code"),
    solutionCodeV4: text("solution_code_v4"),
  ...
  (table) => [index("idx_tour_steps_lesson").on(table.lessonId)]
);
...
export const patternBookmarks = pgTable(
  "pattern_bookmarks",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    userId: uuid("user_id")
