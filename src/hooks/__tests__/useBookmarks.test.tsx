...
import { _resetBookmarkLoaderState, useBookmarks } from "@/hooks/useBookmarks";
...
describe("useBookmarks", () => {
  ...
  it("starts with empty bookmarkedIds and transitions isLoading to false", async () => {
    const { result } = renderHook(() => useBookmarks(false), { wrapper });
  ...
  it("loads authenticated bookmarks from API and merges with localStorage", async () => {
    ...
    const { result } = renderHook(() => useBookmarks(true), { wrapper });
    ...
    const syncReq = fake.captured.find((r) => r.url === "/api/bookmarks/sync");
    ...
    it("also calls remove API for logged-in user", async () => {
      ...
      await waitFor(() => {
        const delReq = fake.captured.find(
          (r) => r.url === "/api/bookmarks/p1" && r.init?.method === "DELETE"
        );
