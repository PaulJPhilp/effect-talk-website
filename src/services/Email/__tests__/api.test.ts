...
describe("Email api", () => {
  describe("sendWaitlistConfirmation", () => {
    it("succeeds with EmailNoOp", async () => {
      const program = Effect.gen(function* () {
        ...
        return yield* svc.sendWaitlistConfirmation(
          "user@example.com",
          "playground"
        );
      });
      const result = await Effect.runPromise(
        program.pipe(Effect.provide(EmailNoOp))
      );
    ...
    it("returns EmailError from a failing layer", async () => {
      const FailingEmail = Layer.succeed(Email, {
        sendWaitlistConfirmation: () =>
          Effect.fail(new EmailError({ message: "Resend API error" })),
  ...
  describe("sendConsultingConfirmation", () => {
    it("succeeds with EmailNoOp", async () => {
      const program = Effect.gen(function* () {
        const svc = yield* Email;
        return yield* svc.sendConsultingConfirmation(
          "jane@example.com",
          "Jane"
        );
      });
    ...
    it("returns EmailError from a failing layer", async () => {
      const FailingEmail = Layer.succeed(Email, {
        ...
        sendConsultingConfirmation: () =>
          Effect.fail(new EmailError({ message: "Network error" })),
        sendFeedbackNotification: () => Effect.void,
      });
      const program = Effect.gen(function* () {
        const svc = yield* Email;
