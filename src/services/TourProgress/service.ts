...
import { patterns, tourLessons, tourProgress, tourSteps } from "@/db/schema";
...
export class TourProgress extends Effect.Service<TourProgressService>()(
  "TourProgress",
  {
          ...
          Effect.tryPromise({
            try: async () => {
              const rows = await db
                .select()
                .from(tourLessons)
                .orderBy(asc(tourLessons.orderIndex));
          ...
          Effect.tryPromise({
            try: async () => {
              ...
              const rows = await db
                ...
                .orderBy(
                  asc(tourLessons.orderIndex),
                  asc(tourSteps.orderIndex)
                );
            ...
            try: async () => {
              ...
              try {
                ...
                function hasMissingPatternsTableError(value: unknown): boolean {
                  ...
                  if (
                    typeof value === "string" &&
                    value.includes("effect_patterns")
                  ) {
                    return true;
                  }
                  return false;
        ...
        bulkUpsertProgress: (
          ...
          Effect.tryPromise({
            try: async () => {
              ...
              const values = [...dedupedProgress.entries()].map(
                ([stepId, status]) => ({
                  userId,
                  stepId,
                  status,
