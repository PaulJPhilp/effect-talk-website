...
describe("Db + Analytics + ApiKeys integration (real DB)", () => {
  ...
  beforeEach(async () => {
    if (!(runIntegrationTests && dbAvailable) || isProductionLikeDb()) {
      return;
    }
    const { db } = await import("../../db/client");
    await db.execute(
  ...
  describe("Waitlist and Analytics", () => {
    it("inserts waitlist signup and tracks event", {
  ...
  describe("Feedback", () => {
    it("inserts feedback", { skip: !runIntegrationTests }, async () => {
      ...
      expect(result.message).toBe(
        "This is at least ten characters of feedback."
      );
      expect(result.name).toBeNull();
      expect(result.created_at).toBeDefined();
    });

    it("inserts feedback with optional name", {
      skip: !runIntegrationTests,
    }, async () => {
      if (!dbAvailable) {
        return;
  ...
  describe("User and API keys", () => {
    it("creates user, API key, lists, and revokes", {
      skip: !runIntegrationTests,
    }, async () => {
      if (!dbAvailable) {
        return;
      ...
      const created = await Effect.runPromise(
        ApiKeysApi.createApiKey(userId, "My Key")
      );
      ...
      const listed = await Effect.runPromise(
        ApiKeysApi.listUserApiKeys(userId)
      );
