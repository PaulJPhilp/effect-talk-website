...
import type { AnalyticsEventType, WaitlistSource } from "@/types/strings";
...
export interface DbService {
  ...
  readonly getPatternById: (
    patternId: string
  ) => Effect.Effect<DbPattern | null, DbError>;
  readonly getRuleById: (
    ruleId: string
  ) => Effect.Effect<DbRule | null, DbError>;
  readonly getUserById: (
    userId: string
  ) => Effect.Effect<DbUser | null, DbError>;
  readonly getUserByWorkosId: (
    workosId: string
  ) => Effect.Effect<DbUser | null, DbError>;
  // --- Analytics ---
  readonly insertAnalyticsEvent: (
    eventType: AnalyticsEventType,
    payload?: Record<string, unknown>
  ) => Effect.Effect<void, DbError>;
  // --- API Keys ---
  readonly insertApiKey: (data: {
    userId: string;
    name: string;
    keyPrefix: string;
    keyHash: string;
  ...
  // --- Consulting ---
  readonly insertConsultingInquiry: (data: {
    name: string;
    email: string;
    role?: string;
    company?: string;
  ...
  // --- Feedback ---
  readonly insertFeedback: (data: {
    name?: string;
    email: string;
    message: string;
  }) => Effect.Effect<Feedback, DbError>;
  // --- Waitlist ---
  readonly insertWaitlistSignup: (
    email: string,
...
export const insertWaitlistSignup = (
  email: string,
  source: WaitlistSource,
  roleOrCompany?: string
) =>
