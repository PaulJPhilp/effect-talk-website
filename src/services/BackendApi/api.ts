...
export interface BackendApiService {
  ...
  readonly fetchPattern: (
    id: string
  ) => Effect.Effect<Pattern | null, BackendApiError>;
  /** Fetch all pattern IDs (for `generateStaticParams` in ISR). */
  readonly fetchPatternIds: () => Effect.Effect<
    readonly string[],
    BackendApiError
  >;
  /** Fetch all patterns (ordered by title). */
  readonly fetchPatterns: () => Effect.Effect<
    readonly Pattern[],
    BackendApiError
  >;
  ...
  readonly fetchRule: (
    id: string
  ) => Effect.Effect<Rule | null, BackendApiError>;
  /** Fetch all rule IDs (for `generateStaticParams` in ISR). */
  readonly fetchRuleIds: () => Effect.Effect<
    readonly string[],
    BackendApiError
  >;
  ...
  readonly searchBackend: (
    q: string
  ) => Effect.Effect<SearchResult, BackendApiError>;
}
