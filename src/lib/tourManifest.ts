...
export type TourExpectedMigrationPolicy =
  | "auto"
  | "review-needed"
  | "unchanged";
export type TourV4ValidationPath =
  | "runtime-approved-rule"
  | "current-docs-exact";
...
export function buildTourManifestStepKey(
  lessonSlug: string,
  orderIndex: number
): string {
...
export function indexTourManifest(
  manifest: TourManifest
): ReadonlyMap<string, IndexedTourManifestStep> {
  ...
  for (const lesson of manifest.lessons) {
    for (const step of lesson.steps) {
      ...
      if (stepMap.has(key)) {
        throw new Error(
          `Tour manifest contains a duplicate step entry for ${lesson.slug} step ${step.orderIndex}`
        );
      }
      stepMap.set(key, {
        lesson,
