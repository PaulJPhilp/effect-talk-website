import {
  countTourManifestSnippets,
  countTourManifestSteps,
  indexTourManifest,
  type TourManifest,
...
export type TourMigrationStatus =
  | "unchanged"
  | "auto-certified"
  | "review-needed";
...
export function buildTourArtifactStepKey(
  lessonSlug: string,
  orderIndex: number
): string {
...
export function indexTourMigrationArtifact(
  ...
  for (const lesson of artifact.lessons) {
    for (const step of lesson.steps) {
      ...
      if (stepMap.has(key)) {
        throw new Error(
          `Tour v4 artifact contains a duplicate step entry for ${lesson.slug} step ${step.orderIndex}`
        );
      }
...
export function validateTourMigrationArtifact(
  ...
  if (
    artifact.transformProfile &&
    artifact.transformProfile !== REQUIRED_TOUR_TRANSFORM_PROFILE
  ) {
    throw new Error(
  ...
  if (artifact.stepCount !== stepCount) {
    throw new Error(
      `Tour v4 artifact step count mismatch: expected ${stepCount}, received ${artifact.stepCount}`
    );
  }
  ...
  for (const [key, entry] of manifestStepMap) {
    ...
    if (!step) {
      throw new Error(
        `Tour v4 artifact is missing ${entry.lesson.slug} step ${entry.step.orderIndex}`
      );
    }
    if (
      step.migrationStatus !== "unchanged" &&
      step.migrationStatus !== "auto-certified" &&
      step.migrationStatus !== "review-needed"
    ) {
      throw new Error(
        `Tour v4 artifact has invalid migration status for ${key}: ${String(step.migrationStatus)}`
      );
    }
    if (
      !(
        step.conceptProvenance?.docsRef &&
        step.conceptProvenance?.filePath &&
        step.conceptProvenance?.contentHash
      )
    ) {
      throw new Error(
        `Tour v4 artifact is missing concept provenance for ${key}`
      );
    }
    if (
      !(
        step.solutionProvenance?.docsRef &&
        step.solutionProvenance?.filePath &&
        step.solutionProvenance?.contentHash
      )
    ) {
      throw new Error(
        `Tour v4 artifact is missing solution provenance for ${key}`
      );
    }
    if (
      step.conceptMatchedPatternIds &&
      !Array.isArray(step.conceptMatchedPatternIds)
    ) {
      throw new Error(
        `Tour v4 artifact has invalid concept matched pattern ids for ${key}`
      );
    }
    if (
      step.solutionMatchedPatternIds &&
      !Array.isArray(step.solutionMatchedPatternIds)
    ) {
      throw new Error(
        `Tour v4 artifact has invalid solution matched pattern ids for ${key}`
      );
    }
    if (
      step.reviewRequiredReasonCodes &&
      !Array.isArray(step.reviewRequiredReasonCodes)
    ) {
      throw new Error(
        `Tour v4 artifact has invalid review-required reason codes for ${key}`
      );
    }
    if (step.conceptMigrationReport) {
      if (!Array.isArray(step.conceptMigrationReport.primitives)) {
        throw new Error(
          `Tour v4 artifact has invalid concept migration report for ${key}`
        );
      }
      if (
        ...
        throw new Error(
          `Tour v4 artifact has invalid concept migration report status for ${key}`
        );
      }
    }
    if (step.solutionMigrationReport) {
      if (!Array.isArray(step.solutionMigrationReport.primitives)) {
        throw new Error(
          `Tour v4 artifact has invalid solution migration report for ${key}`
        );
      }
      if (
        ...
        throw new Error(
          `Tour v4 artifact has invalid solution migration report status for ${key}`
        );
      }
    }
  }
  ...
  for (const lesson of artifact.lessons) {
    for (const step of lesson.steps) {
      ...
      if (!manifestStepMap.has(key)) {
        throw new Error(
          `Tour v4 artifact contains an unexpected step entry for ${lesson.slug} step ${step.orderIndex}`
        );
      }
      if (!stepMap.has(key)) {
        throw new Error(
          `Tour v4 artifact is missing ${lesson.slug} step ${step.orderIndex}`
        );
      }
    }
  }
