...
interface PatternFacetEntry<
  TPattern extends PatternFacetShape = PatternFacetShape,
> {
...
export function computeBrowserFacets<TPattern extends PatternFacetShape>(
  ...
  for (const entry of entries) {
    ...
    const matchesDifficulty =
      filters.activeDifficulty === null ||
      difficulty === filters.activeDifficulty;
    const matchesNew = !filters.activeNewOnly || isNew;

    const inCurrentResult = matchesCategory && matchesDifficulty && matchesNew;
