...
const GOAL_PREFIXES = ["guideline", "rationale", "key insight"];
...
const PATTERN_PREFIXES = ["good example"];
...
const ANTI_PATTERN_PREFIXES = ["anti-pattern", "common mistake"];

function matchesHeadingSet(
  heading: string,
  exact: Set<string>,
  prefixes: string[]
): boolean {
 ...
 */
export function getCachedPatternSections(
  id: string,
  content: string
): PatternSections {
  ...
  if (sectionsCache.size >= SECTIONS_CACHE_MAX) {
    sectionsCache.clear();
  }
