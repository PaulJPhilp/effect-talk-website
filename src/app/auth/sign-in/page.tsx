...
import { getAuthorizationUrl, getCurrentUser, isWorkOSConfigured } from "@/services/Auth"
        ...
        <CardContent className="space-y-4">
          ...
          {!workOSConfigured && (
            <Alert variant="destructive">
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>
                WorkOS authentication is not configured. Please set{" "}
                <code className="text-xs">WORKOS_CLIENT_ID</code> and{" "}
                <code className="text-xs">WORKOS_REDIRECT_URI</code> in your{" "}
                <code className="text-xs">.env.local</code> file.
              </AlertDescription>
            </Alert>
          )}
