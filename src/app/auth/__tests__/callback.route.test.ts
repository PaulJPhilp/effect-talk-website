...
const setSessionCookieMock = vi.fn(() => Promise.resolve());
...
describe("GET /auth/callback", () => {
  ...
  it("honors the state-provided return path", async () => {
    ...
    const response = await GET(
      new NextRequest(
        `http://localhost:3000/auth/callback?code=test&state=${encodeURIComponent(state)}`
      )
    );
  ...
  it("rethrows onSuccess errors after logging", async () => {
    upsertUserMock.mockReturnValueOnce(
      Effect.fail(new Error("db failure")) as never
    );
