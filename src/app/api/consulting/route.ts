...
const ConsultingSchema = Schema.Struct({
  ...
  description: Schema.String.pipe(
    Schema.minLength(10, {
      message: () => "Please provide more detail about your project",
    })
  ),
});
  ...
  try {
    ...
    Effect.runPromise(
      sendConsultingConfirmation(email, name).pipe(
        Effect.catchAll((e) =>
          Effect.logWarning(`Email send failed: ${e.message}`)
        )
      )
    ).catch(() => {
      // Silently ignore
    ...
    Effect.runPromise(trackEvent({ type: "consulting_submitted" })).catch(
      () => {
        // Silently ignore
      }
    );
