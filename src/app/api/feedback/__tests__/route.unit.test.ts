...
describe("POST /api/feedback unit", () => {
  ...
  it("returns 500 when feedback insertion fails", async () => {
    insertFeedbackMock.mockReturnValueOnce(
      Effect.fail(new Error("insert failed")) as never
    );
  ...
  it("swallows notification failures and still returns success", async () => {
    sendFeedbackNotificationMock.mockReturnValueOnce(
      Effect.fail(new Error("email failed")) as never
    );
    const { POST } = await import("@/app/api/feedback/route");
