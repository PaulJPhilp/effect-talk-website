"use client";

import { BookOpen, ChevronDown } from "lucide-react";
import dynamic from "next/dynamic";
import Link from "next/link";
import { Fragment, useMemo, useState, useSyncExternalStore } from "react";
import { TourStepNavigation } from "@/components/tour/TourStepNavigation";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import {
  EMPTY_COMPARE_CODE,
  getTourConceptCode,
  getTourSolutionCode,
  type TourCompareView,
} from "@/lib/tourCompare";
import type { TourMode } from "@/lib/tourMode";
import { cn } from "@/lib/utils";
import type { TourStep as TourStepType } from "@/services/TourProgress/types";

const subscribeNoop = () => {
  return () => {
    // No external subscription; this only gates rendering to the client.
  };
};

const TourCodeRunner = dynamic(
  () =>
    import("@/components/tour/TourCodeRunner").then(
      (module) => module.TourCodeRunner
    ),
  {
    ssr: false,
    loading: () => (
      <div className="flex h-full flex-col items-center justify-center bg-muted/30">
        <div className="text-muted-foreground text-sm">
          Loading code editor...
        </div>
      </div>
    ),
  }
);

interface TourStepProps {
  readonly compareView: TourCompareView;
  readonly completedStepIds: ReadonlySet<string>;
  readonly currentStepIndex: number;
  readonly lessonSlug: string;
  readonly mode: TourMode;
  readonly onStepCompleted: (stepId: string) => void;
  readonly step: TourStepType;
  readonly steps: readonly TourStepType[];
}

type LessonTab = "anti-pattern" | "solution";

function renderTextWithInlineCode(text: string) {
  const normalizedText = text.replace(/\\`/g, "`");
  const parts = normalizedText.split(/(`[^`]+`)/g);
  let cursor = 0;

  return parts.map((part) => {
    const partStart = cursor;
    cursor += part.length;

    if (part.startsWith("`") && part.endsWith("`")) {
      return (
        <code
          className="rounded bg-muted px-1 py-0.5 font-mono text-[0.95em]"
          key={`code-${partStart}-${part.slice(1, 12)}`}
        >
          {part.slice(1, -1)}
        </code>
      );
    }

    return <Fragment key={`text-${partStart}`}>{part}</Fragment>;
  });
}

export function TourStep({
  step,
  lessonSlug,
  steps,
  currentStepIndex,
  completedStepIds,
  onStepCompleted,
  mode,
  compareView,
}: TourStepProps) {
  const codeMode = mode === "compare" ? "v3" : mode;
  const hasSolution = Boolean(getTourSolutionCode(step, codeMode));
  const [selectedTab, setSelectedTab] = useState<LessonTab>("anti-pattern");

  // Radix Tabs use useId() which causes hydration mismatch in Next.js 15.5+ / React 19.2.
  // We gate Tabs to the client without a setState-in-effect pattern.
  const mounted = useSyncExternalStore(
    subscribeNoop,
    () => true,
    () => false
  );

  const instructionParagraphs = useMemo(() => {
    return step.instruction
      .split("\n\n")
      .map((paragraph) => paragraph.trim())
      .filter((paragraph) => paragraph.length > 0)
      .map((paragraph) => paragraph.split("\n").join(" "));
  }, [step.instruction]);

  const isCompareMode = mode === "compare";

  return (
    <div
      className={cn(
        "grid grid-cols-1 gap-0",
        isCompareMode
          ? "xl:grid-cols-[minmax(20rem,26rem)_minmax(0,1fr)]"
          : "md:grid-cols-2"
      )}
    >
      {/* Left: Explanation */}
      <div className="p-5 md:border-r md:p-8">
        <h2 className="mb-4 font-bold text-2xl tracking-tight">{step.title}</h2>

        <div className="space-y-3 text-xl leading-relaxed">
          {instructionParagraphs.map((paragraph) => (
            <p key={paragraph.slice(0, 40)}>
              {renderTextWithInlineCode(paragraph)}
            </p>
          ))}
        </div>

        {/* Hints */}
        {step.hints && step.hints.length > 0 && (
          <div className="mt-5">
            <ul className="list-disc space-y-1 pl-5 text-lg text-muted-foreground">
              {step.hints.map((hint) => (
                <li key={hint}>{renderTextWithInlineCode(hint)}</li>
              ))}
            </ul>
          </div>
        )}

        {/* Related Pattern */}
        {step.pattern_id && (
          <div className="mt-4 flex flex-wrap items-center gap-2">
            <Link
              className="flex items-center gap-1.5 text-muted-foreground text-sm transition-colors hover:text-foreground"
              href={`/patterns/${step.pattern_id}?from=${encodeURIComponent(lessonSlug)}&step=${step.order_index}`}
            >
              <BookOpen className="h-4 w-4" />
              View related pattern
            </Link>
            {step.pattern_new && (
              <span className="inline-flex items-center rounded border border-success/50 bg-success/10 px-1.5 py-0.5 font-medium text-success text-xs">
                New
              </span>
            )}
          </div>
        )}

        {/* Navigation */}
        <div className="mt-5 border-t pt-4">
          <TourStepNavigation
            completedStepIds={completedStepIds}
            currentStepId={step.id}
            currentStepIndex={currentStepIndex}
            lessonSlug={lessonSlug}
            mode={mode}
            onMarkCurrentStepComplete={onStepCompleted}
            steps={steps}
          />
        </div>
      </div>

      {/* Right: Code Runner */}
      <div
        className="flex flex-col items-stretch bg-muted/30 px-1 py-1 md:px-2 md:py-2"
        style={{ minHeight: "600px" }}
      >
        <div
          className="flex min-h-[500px] w-full flex-1 flex-col"
          style={{ minHeight: "600px" }}
        >
          {isCompareMode ? (
            <CompareModePanel compareView={compareView} step={step} />
          ) : (
            <TabbedLessonPanel
              hasSolution={hasSolution}
              mode={mode}
              mounted={mounted}
              selectedTab={selectedTab}
              setSelectedTab={setSelectedTab}
              step={step}
            />
          )}
        </div>
      </div>
    </div>
  );
}

interface TabbedLessonPanelProps {
  readonly hasSolution: boolean;
  readonly mode: Exclude<TourMode, "compare">;
  readonly mounted: boolean;
  readonly selectedTab: LessonTab;
  readonly setSelectedTab: (tab: LessonTab) => void;
  readonly step: TourStepType;
}

function TabbedLessonPanel({
  step,
  hasSolution,
  selectedTab,
  setSelectedTab,
  mounted,
  mode,
}: TabbedLessonPanelProps) {
  const conceptCode = getTourConceptCode(step, mode);
  const solutionCode = getTourSolutionCode(step, mode);
  const solutionLabel = mode === "v4" ? "Solution (v4 beta)" : "Solution";

  if (!conceptCode) {
    return (
      <div className="flex h-full items-center justify-center bg-muted/30 p-8 text-center">
        <p className="text-muted-foreground text-sm">
          No code snippet available for this step.
        </p>
      </div>
    );
  }

  if (!(hasSolution && solutionCode)) {
    return (
      <div
        className="w-full flex-1"
        style={{ minHeight: "500px", height: "100%" }}
      >
        <TourCodeRunner
          code={conceptCode}
          panelTitle={mode === "v4" ? "v4 beta lesson code" : "Lesson code"}
          readOnly={true}
        />
      </div>
    );
  }

  if (!mounted) {
    return (
      <div className="flex min-h-0 flex-1 flex-col">
        <div className="h-8 w-full shrink-0 border-b bg-transparent" />
        <div className="mt-0 min-h-0 flex-1">
          <div className="h-full min-h-[500px]">
            <TourCodeRunner
              code={conceptCode}
              panelTitle={mode === "v4" ? "v4 beta preview" : "Lesson code"}
              readOnly={true}
            />
          </div>
        </div>
      </div>
    );
  }

  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <Tabs
        className="flex min-h-0 flex-1 flex-col"
        onValueChange={(value) => {
          if (value === "anti-pattern" || value === "solution") {
            setSelectedTab(value);
          }
        }}
        value={selectedTab}
      >
        <TabsList
          className="h-9 w-full shrink-0 justify-start gap-0 rounded-none border-b bg-transparent p-0"
          variant="line"
        >
          <TabsTrigger
            className="px-3 py-2 text-sm after:hidden data-[state=active]:font-semibold"
            value="anti-pattern"
          >
            Anti-pattern
          </TabsTrigger>
          <TabsTrigger
            className="px-3 py-2 text-sm after:hidden data-[state=active]:font-semibold"
            value="solution"
          >
            {solutionLabel}
          </TabsTrigger>
        </TabsList>
        <TabsContent className="mt-0 min-h-0 flex-1" value="anti-pattern">
          <div className="h-full min-h-[500px]">
            {selectedTab === "anti-pattern" ? (
              <TourCodeRunner
                code={conceptCode}
                panelTitle="Anti-pattern"
                readOnly={true}
              />
            ) : null}
          </div>
        </TabsContent>
        <TabsContent className="mt-0 min-h-0 flex-1" value="solution">
          <div className="flex h-full min-h-[500px] flex-col">
            {solutionCode && selectedTab === "solution" && (
              <TourCodeRunner
                code={solutionCode}
                panelTitle={mode === "v4" ? "v4 beta solution" : "Solution"}
                readOnly={true}
              />
            )}
            {step.feedback_on_complete && (
              <div className="border-t bg-muted/30 px-3 py-2">
                <p className="text-muted-foreground text-sm italic">
                  {step.feedback_on_complete}
                </p>
              </div>
            )}
          </div>
        </TabsContent>
      </Tabs>
    </div>
  );
}

interface CompareModePanelProps {
  readonly compareView: TourCompareView;
  readonly step: TourStepType;
}

function CompareModePanel({ step, compareView }: CompareModePanelProps) {
  const bothEmpty =
    compareView.identical && compareView.v3Code === EMPTY_COMPARE_CODE;

  if (bothEmpty) {
    return (
      <div className="flex h-full min-h-[500px] items-center justify-center p-8">
        <div className="max-w-sm text-center">
          <p className="font-medium text-muted-foreground text-sm">
            No code snippets are available for comparison on this step yet.
          </p>
        </div>
      </div>
    );
  }

  return (
    <div className="grid h-full min-h-[500px] grid-cols-1 gap-3 xl:grid-cols-[minmax(0,1fr)_18rem_minmax(0,1fr)]">
      <div className="min-h-[500px]">
        <TourCodeRunner
          code={compareView.v3Code}
          panelTitle={
            compareView.selectedSnippet === "solution"
              ? "v3 solution"
              : "v3 anti-pattern"
          }
          readOnly={true}
        />
      </div>
      <div className="flex items-stretch">
        <CompareSummaryCard compareView={compareView} step={step} />
      </div>
      <div className="min-h-[500px]">
        <TourCodeRunner
          code={compareView.v4Code}
          panelTitle={
            compareView.selectedSnippet === "solution"
              ? "v4 solution"
              : "v4 anti-pattern"
          }
          readOnly={true}
        />
      </div>
    </div>
  );
}

interface CompareSummaryCardProps {
  readonly compareView: TourCompareView;
  readonly step: TourStepType;
}

function CompareSummaryCard({ step, compareView }: CompareSummaryCardProps) {
  const badge =
    step.migration_status === "review-needed" ? (
      <span className="inline-flex items-center rounded-full border border-rose-300/70 bg-rose-100/80 px-2 py-0.5 font-medium text-rose-900 text-xs">
        Review required
      </span>
    ) : compareView.identical ? (
      <span className="inline-flex items-center rounded-full border border-amber-300/70 bg-amber-100/80 px-2 py-0.5 font-medium text-amber-900 text-xs">
        Unchanged from v3
      </span>
    ) : (
      <span className="inline-flex items-center rounded-full border border-sky-300/70 bg-sky-100/80 px-2 py-0.5 font-medium text-sky-900 text-xs">
        Auto-certified v4 migration
      </span>
    );

  if (compareView.identical) {
    return (
      <div className="h-full w-full rounded border border-border bg-background/90 p-4">
        <div className="mb-3 flex items-center justify-between gap-3">
          <p className="font-semibold text-foreground text-sm">
            Change summary
          </p>
          {badge}
        </div>
        <details aria-label="Identical lesson note" className="group">
          <summary className="flex cursor-pointer list-none items-center justify-between gap-2 rounded-md border border-border border-dashed px-3 py-2 text-muted-foreground text-sm">
            <span>v3 and v4beta are identical for this step.</span>
            <ChevronDown className="h-4 w-4 transition-transform group-open:rotate-180" />
          </summary>
          <p className="mt-3 text-muted-foreground text-sm leading-6">
            {compareView.changeSummary}
          </p>
          {(step.v3_source_ref || step.v3_source_path) && (
            <p className="mt-3 text-muted-foreground text-xs leading-5">
              Source: {step.v3_source_ref ?? "pinned v3 docs"}{" "}
              {step.v3_source_path ? `· ${step.v3_source_path}` : ""}
            </p>
          )}
        </details>
      </div>
    );
  }

  return (
    <div className="h-full w-full rounded border border-border bg-background/90 p-4">
      <div className="mb-3 flex items-center justify-between gap-3">
        <p className="font-semibold text-foreground text-sm">Change summary</p>
        {badge}
      </div>
      <p className="text-muted-foreground text-sm leading-6">
        {compareView.changeSummary}
      </p>
      <div className="mt-3 space-y-1 text-muted-foreground text-xs leading-5">
        <p>
          Anti-pattern: {compareView.conceptIdentical ? "unchanged" : "changed"}
        </p>
        <p>
          Solution: {compareView.solutionIdentical ? "unchanged" : "changed"}
        </p>
        {(step.v3_source_ref || step.v3_source_path) && (
          <p>
            Source: {step.v3_source_ref ?? "pinned v3 docs"}{" "}
            {step.v3_source_path ? `· ${step.v3_source_path}` : ""}
          </p>
        )}
      </div>
    </div>
  );
}
