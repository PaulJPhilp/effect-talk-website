...
describe("TourModeSwitcher", () => {
  ...
  it("marks the active mode from the URL", () => {
    ...
    expect(screen.getByRole("link", { name: "v4" })).toHaveAttribute("aria-current", "page")
    expect(screen.getByRole("link", { name: "v3" })).not.toHaveAttribute("aria-current")
  })
  ...
  it("preserves existing params when generating mode links", () => {
    ...
    expect(screen.getByRole("link", { name: "v3 ↔ v4" }).getAttribute("href")).toBe(
      "/tour/structured-concurrency?step=2&mode=compare"
    )
  })
  ...
  it("renders v4 controls as disabled buttons for guests", () => {
    ...
    expect(screen.getByRole("link", { name: "v3" })).toHaveAttribute("aria-current", "page")
