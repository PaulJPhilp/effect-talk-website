...
function percentile(sortedValues: readonly number[], p: number): number {
  ...
  const index = Math.min(
    ...
    Math.floor(sortedValues.length * p)
  );
...
async function main(): Promise<void> {
  ...
  if (!Number.isFinite(samples) || samples <= 0) {
    throw new Error(
      `TOUR_SAMPLES must be a positive integer, received: ${process.env.TOUR_SAMPLES}`
    );
  }

  console.log(
    `[tour-baseline] base-url=${baseUrl} samples=${samples} sample-slug=${sampleSlug}`
  );
  console.log(
    "[tour-baseline] Make sure `bun run dev` is running in another terminal"
  );
