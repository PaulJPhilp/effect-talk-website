...
import { contentDeployments, patterns, tourLessonsStaging, tourStepsStaging } from "../src/db/schema"
import { loadTourManifest } from "../src/lib/tourManifest"
import { buildTourArtifactStepKey, validateTourMigrationArtifact, type TourMigrationArtifact } from "../src/lib/tourMigrationArtifact"
...
function hasMissingPatternsTableError(error: unknown): boolean {
  ...
  if (typeof error === "string" && error.includes("effect_patterns")) return true
  return false
}
...
async function buildPatternCache() {
  try {
    ...
    patternCache = new Map(allPatterns.map((pattern) => [pattern.title, pattern.id]))
...
async function seed(): Promise<void> {
  ...
  if (!artifactPath) {
    throw new Error("Missing TOUR_V4_ARTIFACT_PATH. Generate the docs-derived tour artifact first.")
  }

  const manifestPath = path.resolve(process.env.TOUR_MANIFEST_PATH ?? path.join(rootDir, "..", "content", "tour", "tour-manifest.json"))
  ...
  for (const lesson of manifest.lessons) {
    ...
    for (const step of lesson.steps) {
      const artifactStep = stepMap.get(buildTourArtifactStepKey(lesson.slug, step.orderIndex))
      if (!artifactStep) {
        throw new Error(`Tour artifact is missing ${lesson.slug} step ${step.orderIndex}`)
      }
