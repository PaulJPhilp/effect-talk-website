...
async function main(): Promise<void> {
  ...
  if (!(baseUrlArg && expectedHostArg)) {
    fail(
      "Usage: bun run scripts/smoke-auth-redirect.ts <base-url> <expected-host>"
    );
  }
  ...
  const authorizeMatch = html.match(
  ...
  const decodedAuthorizeUrl = authorizeMatch[0].replaceAll("&amp;", "&");
  ...
  if (redirectHost !== expectedHostArg) {
    fail(
      `redirect host mismatch. expected=${expectedHostArg} actual=${redirectHost} redirect_uri=${redirectUri}`
    );
  }
