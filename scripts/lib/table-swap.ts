...
export async function validateStaging(
  ...
  for (const table of group.tables) {
    ...
    const tableExists = (exists.rows[0] as Record<string, unknown>)
      ?.table_exists;
    if (!tableExists) {
      throw new Error(
        `Staging table "${stagingTable}" does not exist. Run the seed script first.`
      );
    }

    // Count rows
    const countResult = await db.execute(
      sql.raw(`SELECT count(*) AS cnt FROM "${stagingTable}"`)
    );
    const count = Number(
    ...
    if (count === 0) {
      throw new Error(
        `Staging table "${stagingTable}" is empty. Aborting swap.`
      );
    }
...
export async function swapTables(
  ...
  for (const table of group.tables) {
    statements.push(
      `ALTER TABLE IF EXISTS "${table}" RENAME TO "${table}_retired"`
    );
...
export async function recreateStagingTables(
  ...
  for (const table of group.tables) {
    await db.execute(
      sql.raw(`DROP TABLE IF EXISTS "${table}_staging" CASCADE`)
    );
    await db.execute(
      sql.raw(`CREATE TABLE "${table}_staging" (LIKE "${table}" INCLUDING ALL)`)
    );
  }
}
