...
5. **Tour changes only**: If the release includes tour content, compare-mode logic, or migration-tool inputs, run `bun run qa:tour:v4:release` first. The tour release path starts from `content/tour/tour-manifest.json` plus the pinned v3 source files in `content/tour-docs/v3/`, requires a `pilot-structural` artifact, and carries trust states (`unchanged`, `auto-certified`, `review-needed`) through QA. For staging/prod content promotion, use `bun run tour:prepare:staging[:apply]` or the production equivalent so artifact generation, metadata export, audit, seed, and promote happen in the correct order. Treat `review-needed` as a real release signal, not a cosmetic label.
